...
function seleccionarPokemon(pokemonDiv, p) {
    ...
    // Asignar el Pokémon seleccionado al jugador
    jugadorPokemon = new Pokemon(p.name, p.types[0], p.hp, p.attack, p.defense, [new Move('Ataque Básico', 40)]);
...
function elegirOponente() {
    ...
    return new Pokemon(p.name, p.types[0], p.hp, p.attack, p.defense, [new Move('Ataque Básico', 40)]);
}
...
function actualizarBarraVida(pokemon, barraId) {
    ...
    const porcentajeVida = (pokemon.hpActual / pokemon.hpMax) * 100;
    // Actualizar el ancho de la barra de vida correspondiente
    document.getElementById(barraId).style.width = `${porcentajeVida}%`;
}
...
document.getElementById('btn-ataque').onclick = () => {
    if (jugadorPokemon) {
        ...
        // Si el oponente aún tiene vida, es su turno
...
document.getElementById('btn-curar').onclick = () => {
    if (jugadorPokemon) {
        ...
        actualizarEstado();
        // Actualizar barra de vida
        actualizarBarraVida(jugadorPokemon, 'vida-jugador');
...
function oponenteTurno() {
    ...
    if (accionOponente === 'a') {
        oponentePokemon.attack(oponentePokemon.movimientos[0], jugadorPokemon);
    } else {
        oponentePokemon.heal();
    }
    actualizarEstado();
    actualizarBarraVida(jugadorPokemon, 'vida-jugador');
    verificarFinCombate();
